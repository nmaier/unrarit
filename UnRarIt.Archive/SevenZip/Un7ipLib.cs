...
namespace UnRarIt.Archive.SevenZip
{
    ...
    public class SevenZipArchiveFile : IArchiveFile, IArchiveOpenCallback, IArchiveOpenVolumeCallback, IProgress, IGetPassword, IDisposable
    {
        ...
        readonly FileInfo archive;
        readonly ThreadIOPriority priority;
        FileInfo current;
        Guid format;
        ...
        public SevenZipArchiveFile(FileInfo aArchive, Guid aFormat, ThreadIOPriority aPriority)
        {
            archive = aArchive;
            format = aFormat;
            priority = aPriority;
        ...
        {
            private readonly SevenZipArchiveFile owner;
            ...
            public void GetStream(uint index, out ISequentialOutStream outStream, ExtractMode extractMode)
            {
                ...
                if (extractMode == ExtractMode.Extract)
                {
                    ...
                    SevenZipOutFileStream ostream = new SevenZipOutFileStream(files[index].Destination, (long)files[index].Size, owner.priority);
