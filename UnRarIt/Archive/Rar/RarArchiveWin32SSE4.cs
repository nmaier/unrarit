...
{
    internal class RarArchiveWin32SSE4 : RarArchive
    {
        [DllImportAttribute("unrar_Win32_SSE4.dll", EntryPoint = "RARCloseArchive", CallingConvention = CallingConvention.StdCall)]
        private static extern RarErrors CloseArchiveImpl(IntPtr hArcData);

        [DllImportAttribute("unrar_Win32_SSE4.dll", EntryPoint = "RAROpenArchiveEx", CallingConvention = CallingConvention.StdCall)]
        protected static extern IntPtr OpenArchiveImpl(ref RAROpenArchiveDataEx ArchiveData);

        [DllImportAttribute("unrar_Win32_SSE4.dll", EntryPoint = "RARReadHeaderEx", CallingConvention = CallingConvention.StdCall)]
        protected static extern RarErrors GetHeaderImpl(IntPtr hArcData, ref Header HeaderData);

        [DllImportAttribute("unrar_Win32_SSE4.dll", EntryPoint = "RARProcessFileW", CharSet = CharSet.Unicode, CallingConvention = CallingConvention.StdCall)]
        protected static extern RarErrors ProcessFileImpl(IntPtr hArcData, RarOperation Operation, string DestPath, string DestName);

        [DllImportAttribute("unrar_Win32_SSE4.dll", EntryPoint = "RARGetDllVersion", CallingConvention = CallingConvention.StdCall)]
        public static extern RarErrors GetVersionImpl();

        [DllImportAttribute("unrar_Win32_SSE4.dll", EntryPoint = "RARSetPassword", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.StdCall)]
        protected static extern RarErrors SetPasswordImpl(IntPtr hArcData, string Password);

        [DllImportAttribute("unrar_Win32_SSE4.dll", EntryPoint = "RARSetCallback", CallingConvention = CallingConvention.StdCall)]
        ...
        public RarArchiveWin32SSE4(string FileName, RarOpenMode Mode, UnRarCallback callback) : base(FileName, Mode, callback) { }
    }
}
