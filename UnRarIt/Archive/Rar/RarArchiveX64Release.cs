...
{
    internal class RarArchiveX64Release : RarArchive
    {
        [DllImportAttribute("unrar_x64_Release.dll", EntryPoint = "RARCloseArchive", CallingConvention = CallingConvention.StdCall)]
        private static extern RarStatus CloseArchiveImpl(IntPtr hArcData);

        [DllImportAttribute("unrar_x64_Release.dll", EntryPoint = "RAROpenArchiveEx", CallingConvention = CallingConvention.StdCall)]
        protected static extern IntPtr OpenArchiveImpl(ref RAROpenArchiveDataEx ArchiveData);

        [DllImportAttribute("unrar_x64_Release.dll", EntryPoint = "RARReadHeaderEx", CallingConvention = CallingConvention.StdCall)]
        protected static extern RarStatus GetHeaderImpl(IntPtr hArcData, ref Header HeaderData);

        [DllImportAttribute("unrar_x64_Release.dll", EntryPoint = "RARProcessFileW", CharSet = CharSet.Unicode, CallingConvention = CallingConvention.StdCall)]
        protected static extern RarStatus ProcessFileImpl(IntPtr hArcData, RarOperation Operation, string DestPath, string DestName);

        [DllImportAttribute("unrar_x64_Release.dll", EntryPoint = "RARGetDllVersion", CallingConvention = CallingConvention.StdCall)]
        public static extern RarStatus GetVersionImpl();

        [DllImportAttribute("unrar_x64_Release.dll", EntryPoint = "RARSetPassword", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.StdCall)]
        protected static extern RarStatus SetPasswordImpl(IntPtr hArcData, string Password);

        [DllImportAttribute("unrar_x64_Release.dll", EntryPoint = "RARSetCallback", CallingConvention = CallingConvention.StdCall)]
        ...
        public RarArchiveX64Release(string FileName, RarOpenMode Mode, UnRarCallback callback) : base(FileName, Mode, callback) { }
    }
}
