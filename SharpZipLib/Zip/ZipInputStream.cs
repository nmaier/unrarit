		...
		int InitialRead(byte[] destination, int offset, int count)
		{
			...
			if (entry.IsCrypted) {
...
#else
				if (password == null) {
					throw new ZipPasswordException("No password set.");
				}
				...
				if (cryptbuffer[ZipConstants.CryptoHeaderSize - 1] != entry.CryptoCheckValue) {
					throw new ZipPasswordException("Invalid password");
				}
				...
				if (csize >= ZipConstants.CryptoHeaderSize) {
					...
					throw new ZipPasswordException(string.Format("Entry compressed size {0} too small for encryption", csize));
				}
#endif				
			} else {
#if !NETCF_1_0
